			...
			if (data.code==1){
				switch (action)
				{
					case "search":				
						...
						$(".result-msg").text(r_l+" "+getTrans("Restaurant found",'restaurant_found') );
...
function Localtion(){
	...
	if( rn  && food ){
		var address = "http://fellybellyapp.com/mobileapp/api/search?address="+rn+"";
		h_23(food,address);
	}
	else if( !food ){
...
function h_24(mrct_id){
	...
	var	ajax_request = $.ajax({
		url: "http://fellybellyapp.com/mobileapp/api/MenuCategory?merchant_id="+mrct_id+"", 
		type: 'post',                  
		async: false,
		dataType: 'jsonp',
		crossDomain: true,
...
function h_25(cat,_m_id){
	...
	var  ajax_request = $.ajax({
		url: "http://fellybellyapp.com/mobileapp/api/getItemByCategory?cat_id="+cat+"&merchant_id="+_m_id+"", 
